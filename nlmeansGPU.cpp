...
void ShowErrorMsg(const char* message)
{
	MessageBox(nullptr, message, "warning", MB_ICONWARNING);
}
...
{
	if (FAILED(device->CreateTexture(width, height, 1, 0, Format, D3DPOOL_SYSTEMMEM, ppTexture, nullptr)))
	{
...
{
	if (FAILED(device->CreateTexture(width, height, 1, Usage, Format, D3DPOOL_DEFAULT, ppTexture, nullptr)))
	{
		ShowErrorMsg("D3DXCreateTextureに失敗");
		return false;
	}
...
IDirect3DTexture9* GetSrcTexture(FILTER* fp, FILTER_PROC_INFO* fpip, int frameIndex)
{
	...
	if (fp->check[0] != 0)
	{
		...
		device->UpdateSurface(src, nullptr, dest, nullptr);
		SAFE_RELEASE(src);
		SAFE_RELEASE(dest);
		tex = SrcTexture[currentCacheIndex].Tex;
	}
...
{
	if (ps[spaceRadius & 1][timeRadius] != nullptr)
	{
...
void releaseWaveletWork()
{
	if (work != nullptr)
		_mm_free(work);
	work = nullptr;
	if (before.dest != nullptr)
		_mm_free(before.dest);
	if (after.dest != nullptr)
		_mm_free(after.dest);
	before.dest = nullptr;
	after.dest = nullptr;
}
...
BOOL func_proc(FILTER* fp, FILTER_PROC_INFO* fpip)
{
	...
	{
		fp->exfunc->get_ycp_filtering_cache_ex(fp, fpip->editp, frameIndex + timeRadius + 1, nullptr, nullptr);
	}
	...
	{
		int threadcount = 0;
		fp->exfunc->exec_multi_thread_func(check_threadcount, &threadcount, nullptr);
